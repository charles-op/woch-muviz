const global = {
  ...
  api: {
    apiKey: "ec9c0723a482dd7ec42fcf83e682bb01",
...
async function fetchAPIData(endpoint) {
  ...
  const response = await fetch(
    `${API_URL}${endpoint}?api_key=${KEY}&language=en-US`
  );
...
async function searchAPIData() {
  const KEY = global.api.apiKey;
  const API_URL = global.api.apiUrl;  showSpinner();
  const response = await fetch(
    `${API_URL}search/${global.search.type}?api_key=${KEY}&language=en-US&query=${global.search.term}&page=${global.search.page}`
  );
  const data = await response.json();
  removeSpinner();
  return data; 
